...
use crusst::path::{HelixPath, LinePath, SpiralPath};
...
fn order0_is_just_the_section() {
    ...
    assert_relative_eq!(
        shape.evaluate(Vector3::new(5.0, 0.0, 0.0)),
        0.0,
        epsilon = 1e-6
    );
}
...
fn order1_cone_eigenform() {
    ...
    assert!(
        base_edge.abs() < 1.0,
        "Base edge should be near surface, got {base_edge}"
    );
...
fn order2_helix_eigenform() {
    ...
    assert!(
        far > 0.0,
        "Point at origin should be outside helix, got {far}"
    );
    ...
    assert!(
        ...
        "Helix mesh should have many vertices"
    );
}
...
fn order3_horn_eigenform() {
    ...
    assert!(
        near_tip > 0.0,
        "Near tip but offset should be outside, got {near_tip}"
    );
    ...
    assert!(
        mesh.vertices.len() > 100,
        "Horn mesh should have many vertices"
    );
}
