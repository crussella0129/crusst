...
fn eigenform_order1_cone() {
    ...
    assert!(
        mesh.indices.len() / 3 > 100,
        "Cone should have >100 triangles"
    );
...
fn eigenform_order2_helix() {
    ...
    assert!(
        mesh.indices.len() / 3 > 200,
        "Helix should have >200 triangles"
    );
...
fn eigenform_order3_horn() {
    let path = SpiralPath::new(|t| 20.0 * (1.0 - t * 0.7), |t| 40.0 * t, 2.0);
    let shape = order3(
        path,
        7.0,
        |t| 1.0 - 0.85 * t,
    ...
    assert!(
        mesh.indices.len() / 3 > 200,
        "Horn should have >200 triangles"
    );
