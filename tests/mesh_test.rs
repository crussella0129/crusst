...
fn sphere_mesh_has_triangles() {
    ...
    assert!(mesh.vertices.len() > 10, "Mesh should have many vertices, got {}", mesh.vertices.len());
    assert!(mesh.indices.len() > 10, "Mesh should have many triangle indices");
    assert_eq!(mesh.indices.len() % 3, 0, "Index count must be multiple of 3");
}
...
fn sphere_mesh_vertices_near_surface() {
    ...
    for v in &mesh.vertices {
        let dist = v.norm();
        assert!((dist - 5.0).abs() < 0.5, "Vertex at distance {dist} should be near radius 5.0");
    }
}
...
fn mesh_normals_computed() {
    ...
    for n in &mesh.normals {
        let len = n.norm();
        assert!((len - 1.0).abs() < 0.01, "Normals should be unit length, got {len}");
    }
}
